...
export const RequestService: React.FC = () => {
  ...
  useEffect(() => {
    if (mapRef.current && originCoords && destinationCoords) {
        const bounds = {
            lng1: Math.min(originCoords.lng, destinationCoords.lng),
            lat1: Math.min(originCoords.lat, destinationCoords.lat),
            lng2: Math.max(originCoords.lng, destinationCoords.lng),
            lat2: Math.max(originCoords.lat, destinationCoords.lat),
        };
        mapRef.current.fitBounds([ [bounds.lng1, bounds.lat1], [bounds.lng2, bounds.lat2] ], { padding: 60, duration: 1000 });
    }
